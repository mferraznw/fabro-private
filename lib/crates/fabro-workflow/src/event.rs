    ...
    pub fn trace(&self) {
        ...
        match self {
            ...
            Self::FallbackTriggered {
                ...
                warn!(
                    stage,
                    from_provider, from_model, to_provider, to_model, reason, "Fallback triggered"
                );
            }
            Self::RetryAttempt {
                stage,
                ...
                warn!(stage, attempt, reason, "Retry attempt");
            }
            Self::ModelDiscovered {
                id,
                provider,
                source,
                ..
            } => {
                info!(id, provider, source, "Model discovered");
            }
...
pub fn event_name(event: &Event) -> &'static str {
    match event {
        ...
        Event::FallbackTriggered { .. } => "fallback.triggered",
        Event::RetryAttempt { .. } => "retry.attempt",
        Event::ModelDiscovered { .. } => "model.discovered",
...
fn stored_event_fields_for_variant(event: &Event) -> StoredEventFields {
    match event {
        ...
        | Event::FallbackTriggered { stage, .. }
        | Event::RetryAttempt { stage, .. } => node_stored_fields(Some(stage.clone())),
...
fn event_body_from_event(event: &Event) -> EventBody {
    match event {
        ...
        Event::FallbackTriggered {
            ...
            reason,
            ..
        } => EventBody::FallbackTriggered(fabro_types::FallbackTriggeredProps {
            from_provider: from_provider.clone(),
            from_model: from_model.clone(),
            to_provider: to_provider.clone(),
            to_model: to_model.clone(),
        ...
        Event::RetryAttempt {
            attempt, reason, ..
        } => EventBody::RetryAttempt(fabro_types::RetryAttemptProps {
            attempt: *attempt,
            reason: reason.clone(),
        }),
        Event::ModelDiscovered {
            ...
            source,
            capabilities,
        } => EventBody::ModelDiscovered(fabro_types::ModelDiscoveredProps {
            id: id.clone(),
            provider: provider.clone(),
            source: source.clone(),
