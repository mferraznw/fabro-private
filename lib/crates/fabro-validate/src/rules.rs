...
fn check_model_known_for_provider(
    ...
    if provider.is_some_and(provider_allows_dynamic_models) {
        return Some(Diagnostic {
            ...
            severity: Severity::Warning,
            message: format!(
                "Unknown LiteLLM model '{model}' {context}; enable `[llm.discovery]` or add it to the catalog"
            ),
            node_id,
            edge: None,
            fix: Some(
                "Run `fabro model discover <id>` or use a statically cataloged model".to_string(),
            ),
        });
    }
    Some(Diagnostic {
...
mod tests {
    ...
    #[test]
    fn node_model_known_rule_warns_for_litellm_dynamic_model_until_discovered() {
        let mut g = minimal_graph();
        let mut node = Node::new("work");
        node.attrs.insert(
            "model".to_string(),
        ...
        assert_eq!(d.len(), 1);
        assert_eq!(d[0].severity, Severity::Warning);
        assert!(d[0].message.contains("Unknown LiteLLM model"));
    }
