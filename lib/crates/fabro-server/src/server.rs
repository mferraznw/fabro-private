...
use fabro_model::{BilledModelUsage, BilledTokenCounts, Model, Provider};
...
use tokio::sync::{
    Mutex as TokioMutex, Notify, RwLock as AsyncRwLock, Semaphore, broadcast, mpsc, oneshot,
};
...
pub struct AppState {
    ...
    pub(crate) litellm_models_cache: TokioMutex<Option<LiteLlmModelsCache>>,
...
pub(crate) fn build_app_state(config: AppStateConfig) -> anyhow::Result<Arc<AppState>> {
    ...
    Ok(Arc::new(AppState {
        ...
        litellm_models_cache: TokioMutex::new(None),
        settings,
...
async fn fetch_litellm_models(state: &AppState) -> anyhow::Result<Vec<Model>> {
    let Some(cache) = litellm_models_cache(state).await? else {
        return Ok(Vec::new());
    };
