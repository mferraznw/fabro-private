...
impl Adapter {
    ...
    fn build_request(&self, url: &str, api_key: Option<&str>) -> fabro_http::RequestBuilder {
        ...
        req.bearer_auth(api_key.unwrap_or(&self.http.api_key))
    }
}
...
fn apply_litellm_headers(
    provider_name: &str,
    headers: &fabro_http::HeaderMap,
    usage: &mut TokenCounts,
) {
    if provider_name != "litellm" {
        return;
    }
    usage.upstream_model = header_value(headers, "x-litellm-model-id");
    usage.cache_hit =
        header_value(headers, "x-litellm-cache-hit").and_then(|value| value.parse::<bool>().ok());
    usage.cost_usd =
        header_value(headers, "x-litellm-response-cost").and_then(|value| value.parse().ok());
    usage.upstream_fallbacks = header_value(headers, "x-litellm-fallbacks")
        .map(|value| {
            value
                .split(',')
                .map(str::trim)
...
fn header_value(headers: &fabro_http::HeaderMap, name: &str) -> Option<String> {
    headers
        .get(name)
...
fn merge_provider_options(
    ...
    for (key, value) in opts_map {
        if provider_name == "litellm" && (key == "api_key" || key == "api_key_env") {
            continue;
        }
...
fn litellm_api_key_override<'a>(
    ...
    provider_options
        .and_then(|options| options.get("litellm"))
...
impl ProviderAdapter for Adapter {
    ...
    async fn complete(&self, request: &Request) -> Result<Response, Error> {
        ...
        let mut req = self
            .build_request(
                &url,
                litellm_api_key_override(&self.provider_name, request.provider_options.as_ref()),
            )
            .json(&api_body);
        ...
        apply_litellm_headers(&self.provider_name, &headers, &mut usage);
    ...
    async fn stream(&self, request: &Request) -> Result<StreamEventStream, Error> {
        ...
        let mut header_usage = TokenCounts::default();
        apply_litellm_headers(&provider_name, http_resp.headers(), &mut header_usage);
...
impl StreamState {
    ...
    fn finish_events(&mut self) -> Vec<StreamEvent> {
        ...
        let usage = self.usage.clone() + self.header_usage.clone();
